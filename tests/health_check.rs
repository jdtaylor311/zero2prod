...
static TRACING: Lazy<()> = Lazy::new(|| {
    ...
    // `get_subscriber`, therefore ther are not the same type. We could work around it,
...
async fn spawn_app() -> TestApp {
    // The first time `initilaze` is invoked the code in `TRACING` is executed.
...
async fn subscribe_returns_a_400_when_fields_are_present_but_empty() {
    ...
    let test_cases = vec![
        ...
        ("name=Urusula&email=", "empty email"),
        (
            "name=Urusula&email=definitely-not-an-email",
            "invalid email",
        ),
    ];
